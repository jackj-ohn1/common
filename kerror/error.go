...
type Error struct {
	...
	code  kcode.Code // Error code if necessary.
}
...
func (err *Error) Code() kcode.Code {
	if err == nil {
		return kcode.CodeNil
	}
	if reflect.DeepEqual(err.code, kcode.CodeNil) {
...
func (err *Error) SetCode(code kcode.Code) {
	if err == nil {
		return
	}
	err.code = code
